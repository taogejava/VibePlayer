...
- 🎚️ **均衡器面板优化** — 重新设计 EQ 滑块样式，垂直条形设计 + 渐变填充 + 发光效果，预设按钮视觉升级
- 📦 **设置面板整合** — 将均衡器功能移入设置面板的「音效」tab，移除顶部独立按钮，简化界面
- ✂️ **移除动效设置** — 动效设置功能移除，保持核心功能专注

### v1.6.0 (2026-05-08)

- 🎨 **主题系统重构** — 选一个颜色，自动生成完整明/暗主题。移除 13 个固定预设，改为 8 款精选色 + 自定义取色器
- 🌓 **显示模式** — 新增浅色/深色/跟随系统/定时切换四种模式，首页右上角一键快速切换
- 🏠 **首页布局优化** — 卡片 4 列自适应网格，7 张卡片更紧凑排列
- 🎚️ **10 段均衡器** — 10 频段 EQ 滑块 + 7 款预设（流行/摇滚/古典/人声/电子/低音增强/高音增强）
- 📊 **听歌报告** — 播放历史可视化统计，播放时长/最爱歌曲/播放分布一图呈现
- 🎹 **系统媒体键** — 支持键盘媒体键（播放/暂停、上一首、下一首），全局响应
...
| 功能 | 描述 |
|------|------|
| 🏠 绚丽首页 | 粒子动画 + 光球漂浮，7 大功能卡片一键直达，4 列自适应布局 |
| 🌌 粒子背景特效 | Canvas 实时渲染的浮动粒子，播放时自动生成，带发光拖尾效果 |
| 💿 黑胶唱片动画 | 播放时自动旋转，暂停时静止，配有锥形渐变纹理和动态光晕 |
| 📊 频谱可视化 | 48 条彩色频谱条，播放时随机律动，颜色跟随歌曲主题变化 |
| 🎚️ 10 段均衡器 | 10 频段 EQ 滑块，7 款预设（流行/摇滚/古典/人声/电子等） |
| 📊 听歌报告 | 播放历史可视化统计，时长/最爱歌曲/播放分布 |
| 🎵 在线歌词搜索 | 本地歌曲无内嵌歌词时，一键搜索在线歌词，支持 LRC 同步滚动 |
| 🎵 在线音乐搜索 | iTunes API 模糊搜索，试听 30s 免版权预览，一键加入播放列表 |
| 📁 本地音乐库 | 选择本地文件夹，递归扫描并按目录树展示，点击即播（10 种格式） |
| 🎬 本地视频播放 | 扫描本地视频文件，完整播放器控件（全屏/倍速/快进快退） |
| 🔗 URL 直链播放 | 粘贴任意音视频 URL 直接播放，自动检测文件类型 |
| 📺 哔哩哔哩播放 | 粘贴 B站链接，解析视频信息，内嵌播放器窗口内直接观看 |
| ☁️ WebDAV 云盘 | 连接群晖/威联通/NextCloud 等网盘，直接播放音视频 |
| 📦 AList 网盘聚合 | 一次对接百度/阿里/123/蓝奏/夸克等所有网盘，点击即播 |
| 🎚️ 完整播放控制 | 进度条拖拽、音量调节、上一首/下一首、播放/暂停、倍速 |
| 🎨 颜色驱动主题 | 选一个颜色，自动生成完整明/暗主题，8 款精选色 + 自定义取色器 |
| 🌓 显示模式 | 浅色/深色/跟随系统/定时切换，首页一键快速切换 |
| 🔤 字体系统 | 6 种中文字体（苹方、思源黑体、站酷快乐体等），一键切换 |
| 🎹 系统媒体键 | 支持键盘媒体键（播放/暂停、上一首、下一首） |
...
VibePlayer 采用**颜色驱动**主题引擎：选一个颜色，自动生成完整的明/暗主题变体。
...
除了内置颜色外，还支持**自定义取色器**：
- 点击「自定义颜色」选择任意颜色
- 自动生成完整的明/暗主题（30+ CSS 变量）

### 显示模式

VibePlayer 支持 **4 种显示模式**：
...
| 🌙 深色模式 | 始终使用暗色主题 |
| 💻 跟随系统 | 自动跟随操作系统明暗偏好 |
| ⏰ 定时切换 | 设定时间段自动切换（如 18:00-07:00 深色） |

> 💡 **提示**：首页右上角有 ☀️/🌙 快速切换按钮，一键切换浅色/深色！
...
|------|------|----------|
| **系统默认** | 跨平台最佳兼容 | 日常使用 |
| **苹方** | Apple 设计语言 | macOS/iOS 风格 |
| **Noto Sans SC** | Google 开源字体 | 多平台一致性 |
| **思源黑体** | Adobe 开源字体 | 专业排版 |
| **站酷快乐体** | 手写风格可爱字体 | 轻松活泼场景 |
| **马善政楷体** | 中国风楷书 | 传统文艺风格 |

### 如何切换？

1. 点击顶部导航栏的 **⚙️ 设置按钮**
2. 在设置面板中选择 **🎨 颜色** / **🌓 显示** / **🔤 字体** 三个标签页
3. 点击喜欢的颜色/模式/字体即可**即时生效**

> 💡 **提示**：首页右上角 ☀️/🌙 按钮可快速切换浅色/深色模式！
...
│   │   ├── EQPanel.tsx             # 10 段均衡器面板
│   │   ├── ListeningReport.tsx     # 听歌报告
...
│   │   ├── useAudioContext.ts       # 共享 AudioContext + EQ 引擎
